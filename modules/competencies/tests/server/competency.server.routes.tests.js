 ...
 * Globals
 */
var app, agent, credentials, user, competency;
...
describe('Competency CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/competencies')
					...
					.end(function(competencySaveErr, competencySaveRes) {
						...
						agent.put('/api/competencies/' + competencySaveRes.body._id)
							.send(competency)
							.expect(200)
	...
	it('should be able to get a list of Competencies if not signed in', function(done) {
		...
		competencyObj.save(function() {
			...
			request(app).get('/api/competencies')
	...
	it('should be able to get a single Competency if not signed in', function(done) {
		...
		// Save the Competency
		competencyObj.save(function() {
			request(app).get('/api/competencies/' + competencyObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/competencies')
					...
					.end(function(competencySaveErr, competencySaveRes) {
						...
						agent.delete('/api/competencies/' + competencySaveRes.body._id)
							.send(competency)
							.expect(200)
	...
	it('should not be able to delete Competency instance if not signed in', function(done) {
		...
		competencyObj.save(function() {
			...
			request(app).delete('/api/competencies/' + competencyObj._id)
			.expect(403)
			.end(function(competencyDeleteErr, competencyDeleteRes) {
				// Set message assertion
				(competencyDeleteRes.body.message).should.match('User is not authorized');
	...
	afterEach(function(done) {
		User.remove().exec(function(){
			Competency.remove().exec(function(){
				done();
			});
		});
	});
